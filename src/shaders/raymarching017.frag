...
float map(vec3 p) {
    ...
    float torusDist = sdTorus(p, vec2(1.0, 0.4));
    
    vec3 spherePos = vec3(0.0, 1.2 * sin(iTime * 0.7), 0.0);
    float sphereDist = sdSphere(p - spherePos, 0.8);
    
    
    return max(sphereDist, -torusDist);
}
