...
float map(vec3 p) {
    ...
    float sphereSDF = length(p) - dynamic_radius;
    
    vec3 boxPos = p - vec3(0.0, 0.0, 0.0);
    float boxSDF = sdBox(boxPos, vec3(0.7, 0.6, 1.0 * abs(tan(iTime * 1.5))));
    
    //return min(sphereSDF, boxSDF); // union
    //return max(sphereSDF, boxSDF); // intersection
    return max(sphereSDF, -boxSDF); // subtruction
}
