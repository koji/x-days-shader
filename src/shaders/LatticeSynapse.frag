...
void main()
{
    ...
    for (int i = 0; i < 100; i++) {
        ...
        if (d < 0.001) {
            float scanWave = sin(p.y * 2.0 -iTime * 3.0);
            float line = smoothstep(0.9, 0.95, scanWave);
            vec3 lineColor = vec3(0.25, 1.0, 0.3);
            
            vec3 finalColor = mix(LED_COLOR * 0.5, lineColor, line);
