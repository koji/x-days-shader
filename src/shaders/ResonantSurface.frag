...
float spikes(vec3 p) {
    float spike_freq = 8.0 * cos(iTime) + 8.0; 
    float spike_amp = 0.15;
    vec3 q = p * spike_freq;
    return (sin(q.x) * sin(q.y) * sin(q.z) + cos(iTime * 0.345)) * spike_amp;
}


float map(vec3 p) {
    float distTetrahedron = sdTetrahedron(p, 1.0);
    float spikeValue = spikes(p);
    distTetrahedron -= spikeValue;
